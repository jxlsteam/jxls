 ...
 *
 * @author Leonid Vysochyn Date: 2/3/12
 */
public class Util {
  ...
  public static final String regexCellRef =
      "([a-zA-Z]+[a-zA-Z0-9]*![a-zA-Z]+[0-9]+|(?<!\\d)[a-zA-Z]+[0-9]+|'[^?\\\\/:'*]+'![a-zA-Z]+[0-9]+)";
  ...
  private static final Pattern regexCellRefExcludingJointedPattern =
      Pattern.compile(regexCellRefExcludingJointed);
  ...
  public static Boolean isConditionTrue(
      ExpressionEvaluator evaluator, String condition, Context context) {
  ...
  public static Boolean isConditionTrue(ExpressionEvaluator evaluator, Context context) {
    ...
    if (!(conditionResult instanceof Boolean)) {
      throw new EvaluationException(
          "Condition result is not a boolean value - " + evaluator.getExpression());
    }
    return (Boolean) conditionResult;
  }

  public static void setObjectProperty(
      Object obj, String propertyName, String propertyValue, boolean ignoreNonExisting) {
    try {
  ...
  public static List<CellRef> createTargetCellRefListByColumn(
